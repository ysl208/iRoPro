...
void Editor::AddGripperPoseAction(const std::string& db_id, size_t step_id,
    ...
    for (size_t height = 0; height < spec.height_num; ++height) {
      ...
      for (size_t row = 0; row < grid.size(); ++row) {
        for (size_t col = 0; col < grid[row].poses.size(); ++col) {
          ...
          std::cout << "dy is " << dy << ",";
          std::cout << "dz is " << dz << "\n";
          if (CheckGridPositionFree(world.surface_box_landmarks,
                                    pose.position) ||
              height > 0) {
            ...
            if (!gripperSpace) {
              std::cout
                  << "Gripper space not enough, checking other programs\n";
            ...
            for (size_t id = 0; id < cart_pose_actions.size(); ++id) {
              ...
              std::cout << "New x pose for step/action = " << s_id << ","
                        << a_id << ": x = "
                        << new_program.steps[s_id].actions[a_id].pose.position.x
                        << ", y = "
                        << new_program.steps[s_id].actions[a_id].pose.position.y
  ...
  bool Editor::CheckObjectCollision(
  ...
  bool Editor::CheckGripperSpaceEnough(
    ...
    for (size_t i = 0; i < landmarks.size(); ++i) {
      ...
      std::cout << "AABB result is: " << aabbResult << " for Landmark "
                << lm.name << ". Press Enter to continue. \n";
      std::cin.ignore();
  ...
  bool Editor::CheckGridPositionFree(
      const std::vector<msgs::Landmark>& landmarks,
      const geometry_msgs::Point& position) {
  ...
  void Editor::DeleteAction(const std::string& db_id, size_t step_id,
    ...
    bool success = db_.Get(db_id, &program);
    if (!success) {
      ROS_ERROR("Unable to delete action from program ID \"%s\"",
                db_id.c_str());
      return;
    }
    if (step_id >= program.steps.size()) {
  ...
  void Editor::GetJointValues(const std::string& db_id, size_t step_id,
    ...
    bool success = db_.Get(db_id, &program);
    if (!success) {
      ROS_ERROR("Unable to update action from program ID \"%s\"",
                db_id.c_str());
      return;
    }
    if (step_id >= program.steps.size()) {
  ...
  void Editor::GetPose(const std::string& db_id, size_t step_id,
  ...
  bool Editor::ClosestLandmark(
      const geometry_msgs::Vector3& ee_position, const World& world,
      const double squared_distance_cutoff, msgs::Landmark* landmark) {
    bool success = false;
